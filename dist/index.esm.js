...
const serialize = d => {
  ...
  const str = ["P", s(d.years, "Y"), s(d.months, "M"), s(d.weeks, "W"), s(d.days, "D"), (d.hours || d.minutes || d.seconds) && "T", s(d.hours, "H"), s(d.minutes, "M"), s(d.seconds, "S")].filter(Boolean).join("");
  return str;
};
const parse = durationStr => {
  ...
  for (const idx in durationMatch) {
    if (durationMatch[idx]) {
      const unit = timeMap[idx].u; // @ts-ignore
